...
import com.kaveingas.incomemanager.exception.ApiSubError;
...
public class ValidationUtils {
	...
	private static final String EMAIL_PATTERN = ALL_CHARS_WITH_NO_SPACE + "+" + AT_SYMBOL + LETTERS_AND_NUMBERS + "+"
			+ DOT + "+(" + LETTERS_AND_NUMBERS + "+)";
	...
	public static List<ApiSubError> getFormErrors(BindingResult bindingResult) {
		List<ApiSubError> errors = bindingResult.getFieldErrors().stream().map(fieldError -> {
			
			return new ApiSubError(fieldError.getObjectName(), fieldError.getDefaultMessage(),fieldError.getField(),fieldError.getRejectedValue());
